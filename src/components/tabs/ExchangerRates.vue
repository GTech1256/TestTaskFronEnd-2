...
GET /api/MarketRates/exchangers          Gets all exchangers

 */

const mainLink = 'https://koshelek.ru/api/ExchangerRates';
const pairsLink = `${mainLink}/pairs`;
const exchangersLink = `${mainLink}/exchangers`;
...
export default {
  ...
  watch: {
    async didLoadByMarket(didLoadByMarket) {
      if (didLoadByMarket) {
        ...
        const exchangersResponse = await axios.get(pairsLink);
  ...
  async created() {
    try {
      ...
      const exchangersResponse = await axios.get(exchangersLink);
  ...
  methods: {
    ...
    async loadPairsByMarket() {
      ...
      exchangers = exchangers.replace('/', '%2');
      const link = `${pairsLink}/${exchangers}`;
